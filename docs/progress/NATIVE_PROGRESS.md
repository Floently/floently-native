...
M17-F live backend and real-device QA - M17-F3 Learn access subscription fallback.
...
M17-F3 Learn access subscription fallback was verified locally with debug and release builds.
...
Reinstall the debug APK on the real Android device and verify the real account opens Floently Learn directly without the 404 access gate.
